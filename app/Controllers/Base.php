    ...
    public function convertPdfToText()
{
    ...
    // Gerar um nome aleatório para a pasta mãe
    $parentRandomName = bin2hex(random_bytes(10)); // Isso irá gerar um nome aleatório de 20 caracteres
    $parentPath = './writable/uploads/php/' . $parentRandomName;
    ...
    foreach ($files['pdf_file'] as $file) {
        if ($file->isValid() && !$file->hasMoved()) {
            // Gerar um nome aleatório para a pasta do arquivo
            $randomName = bin2hex(random_bytes(10));

            // Criar o diretório para o arquivo dentro da pasta mãe
            $childPath = $parentPath . '/' . $randomName;
            if (!is_dir($childPath)) {
                mkdir($childPath, 0777, true);
            }

            // Mover o arquivo para o diretório
            $file->move($childPath, $randomName . '.pdf');
    ...
    // Mover a pasta mãe para ./writable/uploads/pdf/
    $finalPath = './writable/uploads/pdf/' . $parentRandomName;
    if (!rename($parentPath, $finalPath)) {
        // Falha ao mover o diretório
        return view('pdf_error', ['error' => 'Erro ao mover o diretório para o local final']);
    }

    // Resto do código conforme sua necessidade ...
